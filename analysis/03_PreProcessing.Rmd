...
asv_negControls_df <- 
  ...
  # Transform counts to relative abundance so that they are at least roughly comparable between samples! 
...
negControl_ASV_statTest_plotting <- 
  negControl_ASV_statTest %>% 
  # Add the x and y positions for plotting 
  add_xy_position(x = "Sample_or_Control")

# Let's make a dot plot
asv_negControls_df %>%
  # build the plot 
  ggplot(aes(x = Sample_or_Control, y = Abundance, 
             fill = Sample_or_Control, color = Sample_or_Control)) + 
  ...
  # finally, let's add the pvalues! 
  stat_pvalue_manual(negControl_ASV_statTest_plotting) + 
