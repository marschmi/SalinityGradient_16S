...
<span style="color: red;">INTERPRETATION #1 of ASV Lengths: Here, in this location of your analyses, please insert a description, similar to the statements above, of your hypotheses of your expected ASV length. Please provide a clear explanation using information on the sequence strategy (e.g. 2x250 Ilummina MiSeq), primer information (e.g., length of primers), library preparation information (e.g., are the primers sequenced? length of amplicon?), and how your filterAndTrim() parameters might have modified the length of your ASVs.</span>
...
<span style="color: red;">INTERPRETATION #2 of ASV Lengths: Here, in this location of your analyses, please insert a description of the interpretation you draw regarding the lengths of your ASVs that you've inferred from the ASV length table function above and it's graphical visualization above. Do these results match with what you stated in INTERPREATAION #1?</span>

***

<span style="color: red;">INTERPRETATION #3 of ASV Lengths: Based on the data from the ASV length table and your answer above in INTERPRETATION #2, what is your suggested trimming procedure for your ASV lengths? Will you allow some variation or trim to an exact length, similar to our in class example here?</span>
...
plot_ASVLength_NoChimeras <- 
  ...
  # include the x-axis scales
  scale_x_continuous(limits = c(0, maxLength_ASV + 5)) + 
  labs(title = "Trimmed + Chimera Removal",
       y = "Number of ASVs", x = "ASV Sequence Length (bps)")
...
```{r numSeqs-histogram, fig.height=2.5, fig.width=7}
plot_ReadDepth <- 
  track_counts_df %>%
  ggplot(aes(x = nochim)) + 
  geom_histogram() + 
  labs(x = "Total # of Sequences", y = "# of Samples",
       title = "Read Depth after DADA2") + 
  theme_bw()
...
plot_ASVRichness <- 
  ...
  labs(x = "Total # of ASVs", y = "# of Samples",
       title = "ASV Richness after DADA2") + 
  theme_bw()
...
<span style="color: red;">INTERPRETATION #4 of Read Depth and ASV Richness: Here, in this location of your analyses, please insert a description of the interpretation you draw regarding the plots you made above for your project data. Please be sure to note anything important about the figure you've created regarding your data including any specific notes and considerations for later. Similar to the example that is provided above.</span> 
